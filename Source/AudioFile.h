...
bool AudioFile<T>::decodeWaveFile(std::vector<uint8_t>& fileData)
{
    ...
    if (audioFormat != WavAudioFormatAudioFIle::PCM && audioFormat != WavAudioFormatAudioFIle::IEEEFloat && audioFormat != WavAudioFormatAudioFIle::Extensible)
    {
    ...
    for (int i = 0; i < numSamples; i++)
    {
        for (int channel = 0; channel < numChannels; channel++)
        {
            ...
            else if (bitDepth == 32)
            {
                ...
                if (audioFormat == WavAudioFormatAudioFIle::IEEEFloat)
...
bool AudioFile<T>::saveToWaveFile(std::string filePath)
{
    ...
    int16_t audioFormat = bitDepth == 32 ? WavAudioFormatAudioFIle::IEEEFloat : WavAudioFormatAudioFIle::PCM;
    int32_t formatChunkSize = audioFormat == WavAudioFormatAudioFIle::PCM ? 16 : 18;
